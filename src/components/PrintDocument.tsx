...
// PDF / 印刷用の固定レイアウト（800px 幅）。
// 各セクションを data-pdf-block で囲み、PDF 生成側でブロック単位に
// ページ送りすることで、セクションが途中で分断されないようにする。
const PrintDocument = forwardRef<HTMLDivElement, { product: Product }>(({ product }, ref) => {
  ...
  const blockStyle: React.CSSProperties = { breakInside: "avoid" };
  ...
  return (
    ...
    >
      {/* タイトル + 商品名 */}
      <div data-pdf-block style={blockStyle}>
      ...
      {/* 画像 + 訴求点 */}
      <div data-pdf-block style={{ ...blockStyle, display: "flex", gap: 12, marginTop: 12 }}>
        <div style={{ flex: 1 }}>
      ...
      {/* 商品説明 */}
      <div data-pdf-block style={{ ...blockStyle, marginTop: 12 }}>
      ...
      <div data-pdf-block style={{ ...blockStyle, marginTop: 8 }}>
        <div style={{ fontSize: 10, color: "#888" }}>{p.footerNote}</div>
      </div>

      {/* 原価表 */}
      <div data-pdf-block style={{ ...blockStyle, marginTop: 16 }}>
        <div style={{ background: "#243e33", color: "#fff", textAlign: "center", fontSize: 18, fontWeight: 700, padding: "10px", letterSpacing: "0.1em" }}>
          原 価 表
        </div>
      ...
      <div data-pdf-block style={{ ...blockStyle, marginTop: 14 }}>
